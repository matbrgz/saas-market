...
class User extends Model{
    ...
    /**
     * Get Notifications for budgets, suppliers & files.
     *
     * @access public
     * @param  integer $userId
     * @return array
     ...
     * 1. counts of budgets, suppliers, files, users
    ...
    public function dashboard(){
        ...
        // 1. count
        $tables = ["orders", "budgets", "suppliers", "files", "users"];
        $stats  = [];
        ...
        $query  = "SELECT * FROM (";
        $query .= "SELECT 'budgets' AS target, description AS title, date, users.name FROM budgets, users WHERE user_id = users.id UNION ";
        $query .= "SELECT 'suppliers' AS target, title, date, users.name FROM suppliers, users WHERE user_id = users.id UNION ";
