...
var events = {
    ...
    */
    pagination: {
        ...
        /**
         * @var integer Acts like the page number for products
         */
        viewMoreCounter: 1,

        /**
         * @var integer Whenever there is a new product created in-place, this will be incremented.
         */
        productsCreated: 0,
        ...
        init: function(){
            ...
            $("ul.pagination a").click(function(e){
                ...
                if(config.curPage === "products" || config.curPage.indexOf("products") > -1){
                ...
                if(config.curPage === "products" || config.curPage.indexOf("products") != -1) {
                    e.preventDefault();
                    events.products.get(pageNumber, events.pagination.productsCreated);
                }
    ...
    budget:{
        ...
        update: function(){
            $("#list-budgets .header .edit").off('click').on('click', function() {

                var budgetBody = $(this).parent().parent().parent().parent();
                var budgetId   = budgetBody.attr("id");
                getBudgetUpdateForm();


                // 1. get the update form merged with the current budgets data
                function getBudgetUpdateForm(){
                    ajax.send("Budget/getUpdateForm", {budget_id: budgetId}, getBudgetUpdateFormCallBack);

                    function getBudgetUpdateFormCallBack(result){
                        if(helpers.validateData(result, budgetBody, "html", "default", "data")){
                            budgetBody.html(result.data);
                            activateCancelBudgetEvent();
                            activateUpdateBudgetEvent();
                        }
                    }
                }

                // 2. if cancel, then go and get the current budgets(regardless of any changes)
                function activateCancelBudgetEvent(){

                    $("#form-update-"+budgetId+" button[name='cancel']").click(function(e){
                        e.preventDefault();
                        ajax.send("Budget/getById", {budget_id: budgetId}, getBudgetByIdCallBack);

                        function getBudgetByIdCallBack(result){
                            if(helpers.validateData(result, budgetBody, "html", "default", "data")){
                                $(budgetBody).after(result.data);
                                $(budgetBody).remove();
                                events.budget.reInit();
                            }
                        }
                    });
                }

                // 3. if update, then update the current budgets and get back the updated one
                function activateUpdateBudgetEvent(){

                    $("#form-update-"+budgetId).submit(function(e){
                        e.preventDefault();
                        ajax.send("Budget/update", helpers.serialize("#form-update-"+budgetId, "budget_id="+budgetId), updateBudgetCallBack);

                        function updateBudgetCallBack(result){
                            if(helpers.validateData(result, budgetBody, "after", "default", "data")){
                                $(budgetBody).after(result.data);
                                $(budgetBody).remove();
                                events.budget.reInit();
                            }
        ...
        delete: function(){
            ...
            $("#list-budgets .header .delete").off('click').on('click', function(e) {
                e.preventDefault();
                ...
                ajax.send("Budget/delete", {budget_id: budgetId}, deleteBudgetCallBack);
                function deleteBudgetCallBack(result){
                    if(helpers.validateData(result, budgetBody, "html", "default", "success")){
                        $(budgetBody).remove();
                    }
                }
            });
    ...
    products: {
        ...
        get: function(pageNumber, productsCreated){

            if(helpers.empty(pageNumber)) pageNumber = 1;
            if(helpers.empty(productsCreated)) productsCreated = 0;

            ajax.send("Products/getAll", {supplier_id: config.supplierId, page: pageNumber,
                products_created: productsCreated}, getProductsCallBack, "#list-products");

            function getProductsCallBack(result){
                if(helpers.validateData(result, "#list-products", "html", "default", "data")){
                    $("#list-products").append(result.data.products);
                    events.products.reInit();
        ...
        create: function(){
            $("#form-create-product").submit(function(e){
                e.preventDefault();
                ajax.send("Products/create", helpers.serialize(this, "supplier_id="+config.supplierId), createProductCallBack, "#form-create-product");
            });
            function createProductCallBack(result){
                if(helpers.validateData(result, "#form-create-product", "after", "default", "data")){
                    $("#list-products .no-data").remove();
                    $("#list-products").append(result.data);

                    $("#form-create-product textarea").val('');

                    // increment number of products created in-place
                    events.pagination.productsCreated++;
                    events.products.reInit();
                }
            }
        },
        update: function(){
            ...
            $("#list-products .header .edit").off('click').on('click', function() {

                var productBody = $(this).parent().parent().parent().parent();
                var productId   = productBody.attr("id");
                getProductUpdateForm();

                // 1. get the update form
                function getProductUpdateForm(){
                    ajax.send("Products/getUpdateForm", {product_id: productId}, getProductUpdateFormCallBack);

                    function getProductUpdateFormCallBack(result){
                        if(helpers.validateData(result, productBody, "html", "default", "data")){
                            productBody.html(result.data);
                            activateCancelProductEvent();
                            activateUpdateProductEvent();
                        }
                    }
                }

                // 2.
                function activateCancelProductEvent(){
                    $("#form-update-"+productId+" button[name='cancel']").click(function(e){
                        e.preventDefault();
                        ajax.send("Products/getById", {product_id: productId}, getProductByIdCallBack);
                        function getProductByIdCallBack(result){
                            if(helpers.validateData(result, productBody, "html", "default", "data")){
                                $(productBody).after(result.data);
                                $(productBody).remove();
                                events.products.reInit();
                            }
                        }
                ...
                // 3.
                function activateUpdateProductEvent(){
                    $("#form-update-"+productId).submit(function(e){
                        e.preventDefault();
                        ajax.send("Products/update", helpers.serialize("#form-update-"+productId, "product_id="+productId), updateProductCallBack);
                        function updateProductCallBack(result){
                            if(helpers.validateData(result, productBody, "after", "default", "data")){
                                $(productBody).after(result.data);
                                $(productBody).remove();
                                events.products.reInit();
                            }
        ...
        delete: function(){
            ...
            $("#list-products .header .delete").off('click').on('click', function(e) {
                e.preventDefault();
                if (!confirm("Are you sure?")) { return; }
                ...
                ajax.send("Products/delete", {product_id: productId}, deleteProductCallBack);
                function deleteProductCallBack(result){
                    if(helpers.validateData(result, productBody, "html", "default", "success")){
                        $(productBody).remove();
                    }
                }
            });
